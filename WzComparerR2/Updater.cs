...
namespace WzComparerR2
{
    ...
    {
        private const string checkUpdateURL = "https://api.github.com/repos/seotbeo/WzComparerR2/releases/latest";

        public Updater()
        {
            string appVersion = Program.ApplicationVersion;
            //if (TryParseApplicationVersion(appVersion, out var version))
            if (TryParseBuildVersion(appVersion, out var version))
            {
        ...
        public GithubAsset AssetZip { get; private set; }
        ...
        public async Task QueryUpdateAsync(CancellationToken cancellationToken = default)
        {
            ...
            if (TryParseBuildVersion(release.Tag_Name, out var releaseVer))
            {
            ...
            if (release.Assets != null)
            {
                foreach (var asset in release.Assets)
                {
                    if (asset.Name != null)
                    {
                        ...
                        else if (asset.Name.Contains("net462"))
                        {
        ...
        {
            var m = Regex.Match(releaseName, @"(\d{6})(\d{2})$");
            if (m.Success 
