...
namespace WzComparerR2.CharaSimControl
{
    public class QuestTooltipRenderer : TooltipRender
    {
        ...
        private Bitmap RenderBase()
        {
            ...
            TextRenderer.DrawText(g2, $"상태: {stateText[state]}" + (this.Quest.Blocked ? " / 퀘스트 시작 불가" : ""), GearGraphics.EquipMDMoris9Font, new Point(21, picH + Margin_top - 26), ((SolidBrush)GearGraphics.QuestBrushDefault).Color, TextFormatFlags.NoPadding);
        ...
        private void DrawRewardItems(Graphics g, QuestReward r, int h)
        {
            ...
            if (r.Exp > 0)
            {
                ...
                if (!CompareMode)
                {
                    ...
                    this.RewardRectnItems.Add(new Tuple<Rectangle, object>(rect, new TooltipHelp("", r.ExpString, true)));
                }
            }
            if (r.Meso > 0)
            {
                ...
                if (!CompareMode)
                {
                    ...
                    if (this.ShowAllStates)
                        rect.Offset(322 * this.Quest.State, 0);
                    this.RewardRectnItems.Add(new Tuple<Rectangle, object>(rect, new TooltipHelp("", r.MesoString, true)));
                }
            }
            if (r.Items.Count > 0)
            {
